...
function AdenBackend(app, rootConfig) {
  ...
  this.isPROD = process.env.NODE_ENV === 'production'
    || (!!!process.env.NODE_ENV && !rootConfig.dev);
  this.isDEV = rootConfig.dev && !this.isPROD;
  ...
  const rootPage = {
    ...
    // TODO: switch template/templateFile, index/indexFile etc. to use short names for settings
    indexFile: 'index.js',
  ...
  this.rootConfig = _.merge(rootPage, rootConfig);
...
AdenBackend.prototype.loadAdenFile = function loadAdenFile(adenPath, enforce) {
  return Promise.resolve().then(() => {
    ...
    this.logger.debug('Loading .aden', { adenFile });

    try {
      fs.accessSync(adenFile, fs.F_OK | fs.R_OK);
      this.logger.debug('Adenfile access fine', { adenFile });
    } catch (ex) {
      if (enforce) {
      ...
      this.logger.debug('No aden file', { adenFile });
      return {};
    }
