...
function AdenBackend(...args) {
  ...
  this.log = logger(this.config.logger).namespace('aden');
...
AdenBackend.prototype.loadAdenFile = function loadAdenFile(adenPath, enforce) {
  return Promise.resolve().then(() => {
    ...
    if (adenFiles.length === 0) {
      if (enforce) {
        this.log.error(`No .server at ${adenPath}! (Try with 'touch .server' or 'aden -h')`);
        process.exit(1);
      }
    ...
    if (adenFiles.length > 1) {
      this.log.warn('Multiple server config files', { adenFiles });
    }
