...
/**
  ...
  She focuses on the packaging and delivery of frontend assets,
...
function AdenBackend(...args) {
  ...
  const settings = args[0] && args[1] ? args[1] || {} : args[0] || {};
  ...
  this.isPROD = process.env.NODE_ENV === 'production'
    || !this.settings.dev;
  this.isDEV = this.settings.dev || !this.isPROD;
  ...
  this
    ...
    .registerKey('defaults', {
      type: KEY_TYPE_APATH,
      config: true,
      default: this.settings.defaults || path.resolve(__dirname, 'pages'),
    });
}
  ...
  return Promise.resolve().then(() => {
    const applied = keys.map((key) => {
      ...
      if (!newKey.value && newKey.default) {
        Object.assign(newKey, { value: newKey.default });
      }
