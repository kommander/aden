 ...
 *
 * TODO: Use this.keys = {}; and .hasOwnProperty to check for existing,
 *       store key settings this.keys = { key1: { type, inherit, build, ... }, ... }
 *       -> createPage(path, this.keys, settings)
 *        -> FS Check
 *        -> setup getters/setters on page object
 *       -> parsePage(pageWithKeysAndSettings)
 *       -> rootPage (derive initial settings or whatever)
 *       -> parseGraph(rootPage):PageGraph
 *        -> rootPage.subpagePaths.map((path) =>
 *             createPage(path, rootPage.keys, rootPage.settings)
 *             -> subpage
 *             -> parseGraph(subPage)
 *             -> rootPage.children.add(subPage)
 *       -> this.build(pageGraph)
 *       -> this.clean(pageGraph)
 *       ->
 *
 */
AdenBackend.prototype.registerKey = function registerKey(name, key = {}) {
  ...
  // TODO: check for overlapping registered KEY_DIST_PATH -> after key resolve
  // TODO: add rootOnly keys (will only be taken from the root .server)
  //       + key.fromEnv = true will make it setable via [ADEN]_DIST_PATH etc.
  // Use Case:
  //  - the status pages settings
  //  - the .dist path
  //  - the public path
  //  - the basepath
  //  - tmp folder
...
AdenBackend.prototype.applyKeys = function applyKeys(page, keys) {
  return Promise.resolve().then(() => {
    const applied = keys.map((key) => {
      ...
      if (!newKey.inherit) {
        if (newKey.type === KEY_FILE_ARRAY) {
          newKey.value = [];
        } else if (
          newKey.type === KEY_CUSTOM
          || newKey.type === KEY_APATH
        ) {
...
AdenBackend.prototype.registerHooks = AdenExtend.registerHooks;
AdenBackend.prototype.registerHook = AdenExtend.registerHook;
