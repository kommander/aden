...
function AdenBackend(...args) {
  ...
  this.app = args.length > 1 && args[0] ? args[0] : express();
  ...
  }, args.length > 1 && args[1] ? args[1] || {} : args[0] || {});
...
// Constants
AdenBackend.prototype.KEY_TYPE_FILE = 'file';
AdenBackend.prototype.KEY_TYPE_FILE_ARRAY = 'filearray';
...
AdenBackend.prototype.registerFile = function registerFile(keyName, regex, opts = {}) {
  ...
  this.registerKey(keyName, _.extend(opts.key, {
    type: opts.multi ? this.KEY_TYPE_FILE_ARRAY : this.KEY_TYPE_FILE,
...
AdenBackend.prototype.applyKeys = function applyKeys(page, keys) {
  return Promise.resolve().then(() => {
    const applied = keys.map((key) => {
      ...
      if (!newKey.inherit) {
        if (newKey.type === 'filearray') {
          newKey.value = [];
        } else if (newKey.type === 'custom') {
...
AdenBackend.prototype.loadAdenFile = function loadAdenFile(adenPath, enforce) {
  return Promise.resolve().then(() => {
    ...
    let adenFileConfig;
    try {
      adenFileConfig = require(adenFile);
...
AdenBackend.prototype.loadPlugin = AdenExtend.loadPlugin;
...
AdenBackend.prototype.destroy = function destroy() {
  this.app = null;
  this.watcher.close();
  singleton = null;
