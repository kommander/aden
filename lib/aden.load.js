    ...
    .map((page) => page.dotFiles.map((file) => Promise.resolve().then(() => {
      ...
      if (fileInfo.isDir) {
        if (file === page.sendDir) {
          ...
          page.send = customSender || page.send;
...
function loadBuild() {
  ...
  return Promise.resolve().then(() => {
    try {
      ...
      return this.walkPages(pages, rootParent, 'pre:deserialize', 'post:deserialize',
        (page /* , parentPage */) =>
          this.loadAdenFile(page.path)
            .then((fileConfig) => {
              // Add webpack loaders
              ...
              return Object.assign(page, {
                send: fileConfig.send || page.send,
              });
            })
      );
    } catch (ex) {
      throw cannot('load', 'the webpack configuration from dist build')
        .because(ex)
