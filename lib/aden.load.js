...
function load(rootConfig) {
  this.logger.debug(`Loading up app ${rootConfig.path}`);

  return this.loadAdenFile(rootConfig.path, true)
    ...
    .then((rootPage) => this.resolvePaths(rootPage.path, rootPage))
    ...
    .then((pages) => this.walkPages(pages, pages[0], null, null, (nextPage, parentPage) =>
      this.resolvePaths(pages[0].resolved.path, nextPage)
...
function loadPages(rootPage) {
  ...
  return Promise.all([this.parsePage(rootPage.resolved.pagesPath, rootParent)]);
}
...
function executeDotFiles(pages) {
  const promises = this.reducePages(pages)
    ...
    .map((page) => page.dotFiles.map((fileInfo) => Promise.resolve().then(() => {
      ...
      const fullFilePath = path.resolve(page.resolved.path, fileInfo.path);
      if (fileInfo.isDir) {
        if (fileInfo.path === page.sendDir) {
        ...
        } else if (fileInfo.path === page.dataDir) {
        ...
        this.logger.debug('Dotfile fileInfo.base === page.sendFile',
          fileInfo.path === page.sendFile, fileInfo.path, page.sendFile);

        if (fileInfo.path === page.sendFile) {
          const customSend = this.loadCustom(fullFilePath);
          Object.assign(page, {
            send: customSend || page.send,
          });
        } else if (fileInfo.path === page.dataFile) {
          const customData = this.loadCustom(fullFilePath);
          Object.assign(page, {
            loadData: customData || page.loadData,
          });
        } else if (fileInfo.path === page.templateEngineFile) {
...
function loadBuild(rootPage) {
  return Promise.resolve().then(() => {
    ...
    try {
      ...
      const pages = require(path.resolve(rootPage.resolved.dist, rootPage.pageStatsDist));

      _.assign(pages[0], rootPage);
...
function resolvePaths(rootPath, page) {
  return Promise.resolve().then(() => {
    const resolved = Object.keys(page)
      ...
      .filter((key) => {console.log(key, page[key]); return typeof page[key] === 'string';})
      .reduce((prev, key) => _.extend(prev, {
        [key]: !path.isAbsolute(page[key])
          ? path.resolve(rootPath, page[key])
          : page[key],
      }), {});
