...
function load(rootConfig) {
  ...
  return this.loadAdenFile(rootConfig.rootPath, true)
    ...
    .then((rootPage) => this.applyHook('pre:load', { rootPage }))
    .then(({ rootPage }) => {
...
function postLoadWalk(pages) {
  return this.walkPages(pages, this.rootConfig, 'pre:walk', 'post:walk', (page, parentPage) => {
    ...
    return Object.assign(page, {
      ...
      indexFile: page.indexFile || parentPage.indexFile,
      templateFile: page.templateFile || parentPage.templateFile,
      styleFile: page.styleFile || parentPage.styleFile,
    });
  });
}
      ...
      if (fileInfo.isDir) {
        if (fileInfo.file === page.sendDir) {
          ...
          const customSender = this.loadCustom(
            path.resolve(pages[0].rootPath, fileInfo.path, 'index.js')
          );
          Object.assign(page, {
        ...
        if (fileInfo.file === page.sendFile) {
          const customSend = this.loadCustom(
            path.resolve(pages[0].rootPath, fileInfo.path)
          );
          Object.assign(page, {
          ...
          const customTemplateEngine = this.loadCustom(
            path.resolve(pages[0].rootPath, fileInfo.path)
          );
          Object.assign(page, {
...
function walkPages(pages, parentPage, preHook, postHook, cb) {
  return Promise.all(pages.map((page) =>
    (preHook
      ? this.applyHook(preHook, { page, parentPage })
    ...
    .then((result) => this.walkPages(result.children || [], result, preHook, postHook, cb)
       ...
       .then((walkedPage) =>
         (postHook
           ? this.applyHook(postHook, { page: walkedPage })
...
function resolvePaths(rootPath, page) {
  return Promise.resolve().then(() => {
    ...
    // TODO: resolve dotfiles here as well
