...
function postLoadWalk(pages) {
  // Iterate file tree again after load,
  ...
  return this.walkPages(pages, this.rootConfig, 'pre:walk', 'post:walk', (page, parentPage) =>
    Object.assign(page, {
      templateEngine: typeof parentPage.templateEngine === 'function'
        ? parentPage.templateEngine : this.getDefaultTemplateEngine(),
...
function walkPages(pages, parentPage, preHook, postHook, cb) {
  return Promise.all(pages.map((page) =>
    (preHook
      ...
      .then((postHook
        ? this.executeHook(postHook, { page, parentPage })
