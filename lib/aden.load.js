...
function postParseLoadSetup(rootPages) {
  return this.walkPages(rootPages, this.rootPage,
...
function parseGraph(page, parentPage) {
  this.log.info(`Parsing graph for ${page.entryName}`, {
    action: 'parseGraph',
      ...
      .forEach((key) => {
        if (key.value || key.value === '') {
          if (key.type === KEY_RPATH
            ...
            const resolved = path.resolve(this.settings.dist, key.value);
            Object.assign(key, {
              resolved,
              dir: path.parse(resolved).dir,
            });
            ...
            key.value.forEach((fileInfo, index) => {
              ...
              if (key.entry !== ENTRY_STATIC) {
                ...
                dist = path.resolve(this.settings.dist, distFileName);
