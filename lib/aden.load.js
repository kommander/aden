...
function postLoadWalk(pages) {
  return this.walkPages(pages, this.rootConfig,
    (page, parentPage) => this.loadAdenFile(page.pagePath)
      ...
      .then(() => {
        ...
        // console.log('post load walk', page.key.path);
...
function loadBuild(rootPage) {
  return Promise.resolve().then(() => {
    ...
    try {
      ...
      const build = require(path.resolve(rootPage.dist, this.config.pageStatsDist));
...
function resolvePaths(rootPath, page) {
  return Promise.resolve().then(() => {
    // console.log('resolving', this.keys);
    // TODO: Move keys to page.keys
    const keyObj = page.keys
      .reduce((obj, key) => {
        if (key.value || key.value === '') {
          ...
          if (key.type === 'rpath' || key.type === 'file') {
            resolved = path.resolve(rootPath, key.value);
          } else if (key.type === 'pagepath') {
            // console.log(page.relativePath);
            resolved = path.resolve(rootPath, page.relativePath || '', key.value);
          }

          if (key.build) {
            distFileName = `.key.${page.entryName}.${key.name.toLowerCase()}`;
            dist = path.resolve(this.rootConfig.dist, distFileName);
          }

          Object.assign(key, {
            resolved, dist, distFileName,
          });
        }
