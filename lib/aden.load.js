...
// those two look like one and the same function, just with a switch. mb simplify?
async function postParseLoadSetup (rootPages) {
  const pages = await this.walkPages(rootPages,
    (walkPage) => this.bootPage(walkPage)
  )
  return { pages }
}

async function postBuildLoadSetup (rootPages) {
  const pages = await this.walkPages(rootPages,
...
async function restoreBuildPage (page) {
  const fileConfig = await this.loadDotServerFile(page.path.resolved)
  Object.assign(page, {
    fileConfig,
    attitudes: this.sortAttitudes(
      page.attitudes.concat(fileConfig.attitudes || [])
    )
  })
  await this.loadAttitudes(page, page.attitudes)
  Object.assign(page, {
    activeAttitudes: page.attitudes
  })
  await this.applyFileConfig(page.fileConfig, page)
  return page
}

async function bootPage (pageToLoad, isBuild) {
  const { page } = await this.applyHook('boot', { page: pageToLoad })
  if (isBuild) {
    return this.restoreBuildPage(page)
  }
  await this.applyHook('pre:load', { page })
  await this.resolvePaths(this.rootPath, page)
  await this.applyHook('load', { page })
  Object.assign(page, {
    greedy: Array.isArray(page.route)
    ? page.route.filter((route) => route.match(/\*/)).length > 0
    : page.route && !!page.route.match(/\*/)
  })
  await this.applyHook('post:load', { page })
  return page
}
...
function loadBuild (rootPage) {
  return Promise.resolve().then(() => {
    ...
    try {
      const webpackStats = nativeRequire(path.resolve(
    ...
    try {
      const build = nativeRequire(path.resolve(
