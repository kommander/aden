...
function setupRoute(router, page, data) {
  return Promise.resolve({ page, routes: [page.route], handlers: [], data })
    ...
    .then((args) => {
      ...
      // TODO: greedyRoutes break /aden dev app
      if ((!page.children || page.children.length === 0)
        && page.greedyRoutes === true) {
        args.routes = args.routes.map((route) => `${route}*`); // eslint-disable-line
      }

      this.routes.push(args.routes);

      // TODO: Make access log optional (use something like morgan)
      args.handlers.unshift((req, res, next) => {
        this.logger.info(`Access ${args.routes} | ${req.path} | ${req.ip}`);
        next();
      });
