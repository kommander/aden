...
// TODO: Scope hooks to page
function executeHook(name, arg) {
  return new Promise((resolve, reject) => {
    const hooksToExec = [];
    for (let i = 0; i < this.hooks[name].length; i++) {
      const theHook = this.hooks[name][i];
      hooksToExec.push(theHook.call(this, arg));
    }
...
function loadPlugin(dirPath, fileName) {
  ...
  if (this.pluginPaths.indexOf(fullFilePath) !== -1) {
    ...
    return Promise.resolve()
      .then(() => null
        // TODO: Make install switch work again
        // if (program.install) {
        //   return this.runNpm(dirPath);
        // }
      )
      .then(() => plugin(this));
  }
