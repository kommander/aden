...
function devWatchListener(event, filePath) {
  ...
  this.logger.debug('FS Watch event', event, filePath, filename);
  ...
  this.devWatchTimeout = setTimeout(() => {
    ...
    this.logger.info('Re-parsing page tree');
    ...
    this.loadPages(this.rootConfig)
      ...
      .then(({ pages, webpackConfigs }) => {
        ...
        console.log(require('util').inspect(webpackConfigs, { depth: null }));
        try {
...
function setupDev(rootPage, webpackConfigs) {
