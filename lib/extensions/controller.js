...
module.exports = (aden) => {
  ...
  aden.registerFile('getPath', /\.get\.jsx?$/, { key: { build: true } });
  aden.registerFile('postPath', /\.post\.jsx?$/, { key: { build: true } });
  aden.registerFile('putPath', /\.put\.jsx?$/, { key: { build: true } });
  aden.registerFile('deletePath', /\.delete\.jsx?$/, { key: { build: true } });
      ...
      .forEach((method) =>
        ...
        webpackConfigs.push({
          ...
          externals: fs.readdirSync(paths.aden_node_modules).map((module) =>
            `commonjs ${module}` && console.log(module)
          ),
        })
      );
  });
