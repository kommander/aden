...
function build(pages, webpackConfigs) {
  this.log.info('Building Aden app', { rootPage: this.rootPage });
    ...
    .then(() => this.compile(webpackConfigs)
      .catch((err) => {
        if (this.isDEV) {
          ...
          this.log.warn('Webpack failed. waiting for changes...');
        } else {
          throw err;
        }
      }))
    ...
    .map((page) =>
      Object.assign(_.omit(page, [
        ...
        keys: page.keys
          ...
          .map((key) => {
            if (key.type === KEY_TYPE_FILE_ARRAY) {
              Object.assign(key, {
...
function writePageStats(pages) {
  if (!this.isDEV) {
    ...
    const filePath = path.resolve(this.rootConfig.dist, this.settings.pageStatsDist);
...
function writeWebpackStats(pages, stats) {
  if (!this.isDEV) {
    ...
    return new Promise((resolve, reject) =>
      fs.writeFile(
        path.resolve(this.rootConfig.dist, this.settings.webpackStatsDist),
        `[${jsonStats}]`,
        (err) => {
          if (err) {
            reject(err);
