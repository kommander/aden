    ...
    .then((stats) => {
      ...
      this.flattenPages(pages)
        .forEach((page) => {
          ...
          if (stats && stats.stats && stats.stats[0].compilation.namedChunks[page.entryName]) {
            console.log('stats.stats[0].compilation.namedChunks.global.files', stats.stats[0].compilation.namedChunks.global.files);
            const mappedCommons = stats.stats[0].compilation.namedChunks.commons.files
              .concat(stats.stats[0].compilation.namedChunks.global.files)
              .filter((file) => (!file.match(/\.map$/)))
              .map((file) => `${this.rootConfig.publicPath}${file}`);
            Object.assign(page, {
              build: stats.stats[0].compilation.namedChunks[page.entryName].files
