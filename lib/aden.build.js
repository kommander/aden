      ...
      this.flattenPages(pages)
        .forEach((page) => {
          ...
          if (stats && stats.stats && stats.stats[0].compilation.namedChunks[page.entryName]) {
            let commonFiles = stats.stats[0].compilation.namedChunks.commons.files.slice();

            if (stats.stats[0].compilation.namedChunks.global) {
              commonFiles = commonFiles.concat(
                stats.stats[0].compilation.namedChunks.global.files);
            }
