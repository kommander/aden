...
async function writePageStats (pages) {
  if (!this.isDEV) {
    ...
    this.log.start('Writing page stats to dist.', { filePath, pagesJson })

    await writeFile(filePath, pagesJson)
...
// Clear dist folders
async function clean (/* pages */) {
  return primraf(this.settings.dist)
}
