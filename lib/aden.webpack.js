...
function generateWebpackConfig(loadedPages) {
  ...
  .then(({ pages, webpackConfig }) => {
    const rootPage = loadedPages[0];

    const paths = {
      distPath: path.resolve(rootPage.resolved.dist, 'public'),
      node_modules: path.resolve(rootPage.resolved.path, 'node_modules'),
    ...
    const includePaths = [
      ...
      path.resolve(rootPage.resolved.path, '../node_modules'),
      path.resolve(rootPage.resolved.path, '../../node_modules'),
      rootPage.resolved.path,
      paths.aden,
      paths.node_modules,
    ];
  ...
  .then(({ pages, webpackConfig, paths }) =>
    this.walkPages(pages, pages[0], null, null, (page) => {
      this.logger.debug('Applying page to webpackConfig', {
        page: _.pick(page, ['name', 'resolved']),
      });

      _.merge(webpackConfig, {
        includePaths: webpackConfig.resolve.root.concat([page.resolved.shared]),
        resolve: {
          root: webpackConfig.resolve.root.concat([page.resolved.path, page.resolved.shared]),
        },
      });
