...
async function generateWebpackConfig (pages) {
  ...
  const frontendConfig = webpackConfigs[0]
  ...
  await this.applyHook('pre:apply', { pages, webpackConfigs, paths })
  // Apply pages
  await this.walkPages(pages, async (page) => {
    ...
    webpackConfigs.forEach((webpackConfig) =>
      this.applyPagePathsToConfig(webpackConfig, page)
    )

    // Gather keys to consider in file watcher
    Array.prototype.push.apply(this.watchKeys, page.keys
    ...
    htmlKeys.forEach((key) => {
      const htmlPlugin = htmlPluginForKey(key, page)
      frontendConfig.plugins.push(htmlPlugin)
    })

    if (
      page.assets.value.length > 0 &&
      htmlKeys.length > 0
    ) {
      const includeAssets = new HtmlIncludeAssets({
        ...
        assets: page.assets.value,
        append: false
      })
      frontendConfig.plugins.push(includeAssets)
    }

    await this.applyHook('apply', { webpackConfigs, page, webpackEntry })
    // Merge webpack config from page .server
    if (typeof page.webpack.value === 'object') {
      Object.assign(frontendConfig,
        webpackMerge(frontendConfig, page.webpack.value))
    }
    ...
    webpackEntry.unshift(path.resolve(__dirname, 'client/index.js'))
    Object.assign(frontendConfig, {
      entry: Object.assign(frontendConfig.entry, {
        [page.entryName]: webpackEntry
      })
    })
  ...
  await this.applyHook('post:apply', { pages, webpackConfigs, paths })
