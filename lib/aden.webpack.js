    ...
    this.walkPages(pages, null, (page) => {
      ...
      if (this.isDEV) {
        ...
        webpackEntry.unshift(
          `${require.resolve('webpack-hot-middleware/client')}?reload=true&path=${
      ...
      return Promise.resolve({ webpackConfigs, page, webpackEntry, paths })
        .then(args => {
          ...
          webpackConfigs.forEach((webpackConfig) => _.merge(webpackConfig, {
            ...
            module: {
              rules: webpackConfig.module.rules.concat(page.key.rules.value.map((rule) => {
          ...
          args.page.keys
            .filter((key) => (
              key.entry === ENTRY_TYPE_STATIC || key.entry === ENTRY_TYPE_DYNAMIC
            ))
            .forEach((key) => {
              const chunks = ['global', args.page.entryName];

              if (args.page.commons) {
              ...
              if (key.value) {
                if (key.type === KEY_TYPE_FILE || key.type === KEY_TYPE_RPATH) {
          ...
          return this.applyHook('apply', args)
            .then((hooked) => {
              ...
              if (typeof hooked.page.key.webpack.value === 'object') {
                Object.assign(webpackConfigs, {
                  0: webpackMerge(webpackConfigs[0], hooked.page.key.webpack.value),
                });
              }
  ...
  .then(({ pages, webpackConfigs /* , paths */ }) => {
    ...
    Object.assign(webpackConfigs[0], {
      plugins: orderedPlugins
        .concat(this.getDefaultWebpackPlugins())
        .concat(webpackConfigs[0].plugins),
    });
    ...
    this.log.debug('Generated Webpack Config', finalConfigs);
