...
function generateWebpackConfig(loadedPages) {
  ...
  .then(({ pages, webpackConfig, paths }) =>
    this.walkPages(pages, pages[0], null, null, (page) => {
      ...
      _.merge(webpackConfig, {
        includePaths: webpackConfig.resolve.root.concat([
  ...
  .then(({ pages, webpackConfig, paths }) => {
    ...
    webpackConfig.plugins.push(new webpack.DefinePlugin({
      __DEV__: this.isDEV,
      // TODO: add this.isPROD and this.isTEST, process.env.NODE_ENV should not be used anywhere
      __PROD__: process.env.NODE_ENV === 'production' || !!!process.env.NODE_ENV ? 'true' : 'false',
