...
function setupApp(rootPage) {
  ...
  const allPages = [rootPage].concat(this.defaultPages);

  return this.applyHook('setup', _.extend({}, this.ui, { pages: allPages }))
    ...
    .then(() => {
      ...
      if (this.notFoundPage && this.notFoundPage.get) {
        this.log.debug('Setting up Not Found Route', {
      ...
      if (this.errorPage && this.errorPage.get) {
        this.log.debug('Setting up Error Route', {
          page: _.pick(this.notFoundPage, [
            'name',
            'path',
          ]),
        ...
        this.log.warn('No Error page.');
        this.app.use((err, req, res, next) => {
          this.log.error('(No Error Page)', err);
          res.status(500);
          res.end('Error');
          return next();
        });
...
function notFoundRoute(req, res, next) {
  ...
  .catch(errPageErr => {
    ...
    this.log.error('404 Page Broken :/', errPageErr);
    process.exit(1);
  });
}
...
function errorRoute(err, req, res, next) {
  ...
  Promise.resolve().then(() => this.errorPage.get(req, res, this.errorPage, err, next))
    .catch(errPageErr => {
      ...
      this.log.error('Error Page Broken :/', errPageErr);
      process.exit(1);
    });
}
  ...
  return Promise.resolve().then(() => {
    ...
    const routes = this.flattenPages(pages)
      .map((page) => {
        ...
        if (!this.errorPage && page.name === pages[0].error) {
          this.log.debug('Choosing Error Page', _.pick(page, [
            'name',
            'path',
          ]));
        ...
        if (!this.notFoundPage && page.name === pages[0].notFound) {
          this.log.debug('Choosing Not Found Page', _.pick(page, [
            'name',
            'path',
          ]));
