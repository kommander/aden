  ...
  return this.applyHook('setup', { app: this.app, rootPage })
    ...
    .then(({ pages }) => {
      pages.forEach((page) => {
        ...
        if (page.name === rootPage.notFound) {
...
function notFoundRoute(req, res, next) {
  ...
  Promise.resolve().then(() => this.notFoundPage.get(req, res, next))
  .catch(errPageErr => {
...
function errorRoute(err, req, res, next) {
  ...
  Promise.resolve().then(() => this.errorPage.get(err, req, res, next))
    .catch(errPageErr => {
...
function loadCustom(filePath, pageScope) {
  ...
  try {
    ...
    return require(filePath)(this, pageScope);
  } catch (ex) {
