...
function setupApp(rootPage) {
  ...
  if (typeof rootPage.poweredBy === 'string') {
  ...
  return this.applyHook('setup', { pages: [rootPage], app: this.app })
    ...
    return Promise.all(routes)
      .then((results) => {
        const pagesAgain = results.map(({ page }) => {
          ...
          // -> should got to configurable allowedMethods

          page.methods.value.forEach((method) => {
            let handlers = page[method] || [];
            console.log('type', typeof handlers, handlers)
            // Send string data
            ...
            if (typeof handlers === 'string') {
              ...
              throw cannot('use', 'method handler')
                .because('Promise wrapped handlers are not resolved yet')
                .addInfo('Working on that');
            }
