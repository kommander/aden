...
function setupApp(rootPage) {
  ...
  return this.setupRoutes(this.router, [rootPage].concat(this.defaultPages))
    .then(() => {
      ...
      return this.applyHook('setup', { app: this.app });
    });
}
...
function setupRoute(router, page) {
  return Promise.resolve({ router, page })
    .then((args) => {
      this.logger.start(`Creating Route ${page.route}`, {
        page: _.pick(args.page, [
          'name', 'route',
        ]),
      });
      return args;
    })
    .then((args) => this.applyHook('setup:route', args));
}
