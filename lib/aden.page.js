      ...
      .then(({ page, files }) => {
        ...
        const parsedPage = files
          ...
          .reduce((prev, fileInfo) => {
            ...
            // frontend/webpack entry points are native first class citizens?
            if (prev.indexFile && fileInfo.file === prev.indexFile) {
              return Object.assign(prev, {
                index: path.relative(prev.rootPath, fileInfo.fullFilePath),
              });
            }
            ...
            // } else if (prev.templateFile && file === prev.templateFile) {
            //   return Object.assign(prev, {
            //     template: path.relative(prev.rootPath, fullFilePath),
            //   });
            // } else if (prev.styleFile && file === prev.styleFile) {
            //   return Object.assign(prev, {
            ...
            )[0]; // TODO: warn if more than one

            if (fileHandler) {
              this.logger.debug('file handler', fileHandler);
