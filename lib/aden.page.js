...
const { KEY_OBJECT } = require('./aden.constants.js');
          ...
          .reduce((prev, fileInfo) => {
            if (fileInfo.isDir) {
              ...
              if (!prev.focusPath) {
                ...
                // TODO: set route = false for paths withing but below focus path
                prev.subpagePaths.push(fileInfo.fullFilePath);
              }
            ...
            const handlers = this.fileHandlers.filter((handler) =>
              handler.matcher({ page, fileInfo })
            );

            if (handlers.length > 0) {
              Object.assign(page.handledFiles, {
  ...
  return Promise.resolve().then(() => {
    page.keys
      ...
      .forEach((key) => Object.assign(
        key,
        {
          value: key.type === KEY_OBJECT && fileConfig[key.name]
