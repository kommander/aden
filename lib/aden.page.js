...
function ensurePathAccess(pagePath) {
  try {
    ...
    throw cannot('access', 'page path').because(ex).addInfo(pagePath);
  }
}
...
function createPage(pagePath, settings = {}) {
  ...
  const relativePath = path.resolve(
    '/', path.relative(this.rootPath, pagePath)
  ).replace(/^\/|^[A-Z]:\\/i, '');

  // TODO: Use relative path as default route _if inside root path_
  const route = path.join('/', relativePath.replace('+', ':')).replace(/\\/g, '/');
  ...
  const keySettings = _.omit(filteredSettings, pagePrimitives);

  const entryName = relativePath.length > 0 
    ? `${relativePath.replace(/\/$/, '')}${path.sep}bundle`
    : 'bundle';
  ...
  Object.keys(keySettings).forEach((key) => {
    basePage.set(key, keySettings[key]);
  });

  return basePage;
}

function registerPage(absPath, settings = {}) {
  if (!path.isAbsolute(absPath)) {
    throw cannot('register', 'page').because('I need an absolute path');
  }

  if (this.isDEV && this.pageCache[absPath]) {
    this.log.info(`Using page from cache for ${absPath}`);
    return this.pageCache[absPath];
  }

  const page = this.createPage(absPath, settings);
  this.pageCache[absPath] = page;
...
function loadPage(absPath, settings) {
  return Promise.resolve()
    .then(() => this.registerPage(absPath, settings))
    .then((newPage) => this.parseGraph(newPage));
}
...
function initFreshPageToParse(page) {
  return this.loadDotServerFile(page.path.resolved)
    ...
    .then(() => this.applyHook('init:page', { page }))
...
function parsePage(basePage, parentPage) {
  ...
  return Promise.resolve(basePage)
    ...
    .then((page) => {
      if (parentPage) {
        Object.assign(page, {
          __parentPageId: parentPage.id,
        });
        return this.inheritKeys(page, parentPage);
      }
      return page;
    })
    ...
    .then(({ page, files }) => {
      this.log.start(`Parsing page ${page.relativePath || page.name}`, { files });
        ...
        .reduce((prev, fileInfo) => {
          if (fileInfo.isDir) {
            ...
            if (!prev.focusPath || pathIsInside(
              ...
              const existing = prev.children.find((child) => (
                child.path.resolved === fileInfo.fullFilePath
              ));
              if (!existing) {
      ...
      return this.applyHook('post:parse', { page: parsedPage, files })
        .then((args) => {
          this.log.success(`Parsed page ${page.relativePath || page.name}`);
          return args.page;
        });
    });
}
  ...
  return Promise.resolve().then(() => {
    page.keys
      ...
      .forEach((key) => page.set(key.name, key.type === KEY_OBJECT && fileConfig[key.name]
        ...
        : (typeof fileConfig[key.name] !== 'undefined'
          ? fileConfig[key.name]
          : key.value
        )));
  ...
  return Promise.resolve()
    ...
    .then(() => {
      ...
      return new Promise((resolve, reject) => {
        rimraf(distPublicPath, (err) => (err ? reject(err) : resolve()))
      });
    })
    .then(() => {
      const distDynamicPath = path.resolve(
        this.settings.dist,
        'dynamic',
        page.relativePath
      );
    ...
    .then(() => {
      ...
      if (page.__parentPageId) {
        const parentPage = this.pagesById[page.__parentPageId];
        parentPage.children.splice(parentPage.children.indexOf(page), 1);
      }
    });
}
