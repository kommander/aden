  ...
  return Promise.resolve().then(() => {
    ...
    return this.loadAdenFile(pagePath)
      .then((fileConfig) => {
        ...
        // TODO: Make sure route is extended if an array
        ...
        return Promise.all(dotFileHooks)
          ...
          .then((args) => {
            ...
            return Promise.all(subpages)
              .then((children) => ({ children, extendedPage: args.page }));
          })
          .then(({ children, extendedPage }) => ({
            page: Object.assign(extendedPage, {
              children: extendedPage.children.concat(children),
            }),
          }))
          .then((args) => {
            this.logger.success(`Parsed page ${args.page.fullFilePath}`, { page: args.page });
...
function applyPageOnWebpack(page) {
  this.logger.start(`Creating webpack entry for ${page.route} (${page.name})`, { page });

  if (page.createEntry === false || (!page.htmlFileFullPath && !page.render
    && !page.index && !page.template && !page.customRender && !page.customData)) {
    return null;
  }
