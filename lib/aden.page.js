...
function parsePage(pagePath, maybeParentPage) {
  return Promise.resolve().then(() => {
    ...
    return this.executeHook('pre:parse', { page: basePage, parentPage })
      ...
      .then(({ page, files }) => {
        ...
        const parsedPage = files.reduce((prev, file) => {
          ...
          if (fileInfo.name.indexOf('.') === 0 && file !== '.aden') {
            prev.dotFiles.push({
              isDir,
              path: path.relative(this.rootConfig.path, fullFilePath),
              fullFilePath,
              name: fileInfo.name,
              file,
            });
          ...
          if (isDir) {
            ...
            if (file === prev.indexFile) {
              return Object.assign(prev, {
                index: path.relative(this.rootConfig.path, fullFilePath),
              });
            } else if (file === prev.templateFile) {
              return Object.assign(prev, {
                template: path.relative(this.rootConfig.path, fullFilePath),
              });
            } else if (file === prev.styleFile) {
              return Object.assign(prev, {
                style: path.relative(this.rootConfig.path, fullFilePath),
              });
            }
            ...
            if (file === 'favicon.ico') {
              return Object.assign(prev, {
                favicon: path.relative(this.rootConfig.path, fullFilePath),
              });
            }
          }
        ...
        return this.executeHook('post:parse', { page: parsedPage, parentPage })
          .then((args) => {
