      ...
      .then(({ page, files }) => {
        ...
        const parsedPage = files.reduce((prev, file) => {
          ...
          if (fileInfo.name.indexOf('.') === 0 && file !== '.aden') {
            prev.dotFiles.push({
              isDir,
              path: path.relative(prev.rootPath, fullFilePath),
              name: fileInfo.name,
              file,
            });
            return prev;
          ...
          if (isDir) {
            this.logger.debug('Is Dir', { path: fullFilePath, focusPath: prev.focusPath });
            ...
            } else if (pathIsInside(path.resolve(prev.rootPath, prev.focusPath), fullFilePath)) {
            ...
            this.logger.debug('Ignoring subpage in favour of focus path', { path: fullFilePath });
            return prev;
          } else if (fileStats.isFile()) {
            // TODO: Use file extension handlers (rename plugins to extensions)
            // Check for explicit entry points
            if (file === prev.indexFile) {
              return Object.assign(prev, {
                index: path.relative(prev.rootPath, fullFilePath),
              });
            } else if (file === prev.templateFile) {
              return Object.assign(prev, {
                template: path.relative(prev.rootPath, fullFilePath),
              });
            } else if (file === prev.styleFile) {
              return Object.assign(prev, {
                style: path.relative(prev.rootPath, fullFilePath),
              });
            }
            ...
            if (file === 'favicon.ico') {
              return Object.assign(prev, {
                favicon: path.relative(prev.rootPath, fullFilePath),
              });
            }
          }
...
function applyPageOnWebpack(page) {
  this.logger.start(`Creating webpack entry ${page.name}`, {
    page: _.pick(page, [
      'name',
      'route',
      'send',
  ...
  if (this.isDEV) {
    ...
    webpackEntry.unshift(`webpack-hot-middleware/client?reload=true&path=${this.config.hmrPath}`);
  }

  return Promise.resolve({ entry: webpackEntry, page })
    .then(args => this.applyHook('pre:entry', args))
    .then(args => {
    ...
    .then(args => this.applyHook('post:entry', args))
