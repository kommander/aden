...
function init(initPath, focusPath) {
  ...
  this.ui = _.extend({
    ...
    'isDEV', 'isPROD', 'flattenPages', 'server', 'settings', 'rootConfig',
  ]));
  ...
  return this.loadAdenFile(rootPath, true)
    .then((fileConfig) => {
      ...
      _.extend(this.rootConfig, _.pick(fileConfig, [
        'error', 'notFound', 'shared',
      ]), {
        rootPath,
        dist: path.resolve(rootPath, fileConfig.dist || '.dist'),
        attitudes: _.merge(this.rootConfig.attitudes, fileConfig.attitudes, this.settings.attitudes),
      });
