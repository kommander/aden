...
function init(initPath, focusPath) {
  ...
  this.ui = _.extend({
    ...
    'name', 'app', 'version', 'rootPath',
    'isDEV', 'isPROD', 'flattenPages',
  ]));
  ...
  return this.loadAdenFile(rootPath, true)
    ...
    .then((rootPage) =>
      this.loadExtensions(this.config.extensionsPath, rootPage.attitudes)
      .then(() => rootPage))
...
function runStartupCallback({ pages }) {
  if (typeof pages[0].startup === 'function') {
    return Promise.resolve()
      .then(() => pages[0].startup({ aden: this.ui }))
      .then(() => ({ pages }));
  }
