...
function buildTask() {
  ...
  return this.loadPlugins(this.config.pluginsPath)
    .then(() => this.loadPages(this.rootConfig))
...
function runProductionTask() {
  return this.loadPlugins(this.config.pluginsPath)
    .then(() => this.loadBuild(this.rootConfig))
    .then((pages) => this.setup(pages))
    .then(({ pages }) => {
      // TODO: get rid of this.rootPage, use pages[0]
      this.rootPage = pages[0];
...
function init(rootPath) {
  ...
  this.logger.debug('Initializing Aden', rootPath, this.config);
