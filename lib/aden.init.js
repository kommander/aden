...
function runDevTask() {
  return this.loadPages(this.rootConfig)
    ...
    .then(({ pages, webpackConfigs }) => this.setupDev(pages[0], webpackConfigs))
    .then(() => {
      if (typeof this.rootPage.startup === 'function') {
        return this.rootPage.startup({ aden: this });
      }
...
function runProductionTask() {
  return this.loadBuild(this.rootConfig)
    ...
    .then(({ pages }) => {
      if (typeof this.rootPage.startup === 'function') {
        return Promise.resolve()
          .then(() => this.rootPage.startup({ aden: this }))
          .then(() => ({ pages }));
      }
...
function init(initPath, focusPath) {
  if (process.__ADEN__) {
    this.logger.error('Fatal: Only call aden.init() once per process.');
