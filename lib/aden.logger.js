...
Logger.prototype.namespace = function namespace(name, opts = {}) {
  ...
  const stdStream = (string) => options.stdStream.write(`${string}${EOL}`);
  const errStream = (string) => options.errStream.write(`${string}${EOL}`);
  ...
  // TODO: optionally take error as first argument only and omit msg
  ...
  // TODO: Take timing for start/success
...
LogParser.prototype._detach = function detach(item) {
  item.readStream.removeListener('data', item.handler);
};

LogParser.prototype.destroy = function destroy() {
  this.streams.forEach((item) => this._detach(item));
  this.streams = null;
};
...
LogParser.prototype.getStreamHandler = function getStreamHandler() {
  ...
  return (data) => {
    ...
    const lines = stringBuffer.split(EOL);
    if (data[data.length - 1] !== EOL) {
      buf = lines.pop();
    }
    lines.forEach((line) => {
      ...
      if (lineType) {
        ...
        if (lineType === 'event') {
          ...
          if (typeof json.err === 'object') {
            ...
            const errLines = json.err.split(EOL);
          ...
          if (err && err.message && err.message.match(/Worker error code/i)) {
