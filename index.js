...
program
  ...
  // .option('--clean', 'Remove all loaded dependencies and tmp folders in the tree')
  // .option('--focus', 'Choose one route to focus on. Mount only that.')
  .parse(process.argv);
...
bootstrap(app, logger, {}).then((result) => {
  const aden = result[0];
